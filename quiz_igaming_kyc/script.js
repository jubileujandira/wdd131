...
function sendResult(status, score, correct, details, useBeacon = false) {
  const name = participantName.value.trim() || "Não informado";
  const payload = new URLSearchParams();
  payload.set("_subject", "Resultado Quiz iGaming - Cadastro/KYC");
  payload.set("_captcha", "false");
  payload.set("_template", "table");
  payload.set("Nome", name);
  payload.set("Nota", `${score}/${questions.length}`);
  payload.set("Acertos", String(correct));
  payload.set("Status", status);
  payload.set("DataHora", nowBR());
  payload.set("Detalhes", details);

  const url = "https://formsubmit.co/leandroarcres@gmail.com";
  ...
  document.getElementById("mailScore").value = `${score}/${questions.length}`;
  document.getElementById("mailCorrect").value = String(correct);
  document.getElementById("mailStatus").value = status;
  document.getElementById("mailDate").value = nowBR();
  document.getElementById("mailDetails").value = details;
...
function finishNormally() {
  ...
  sendResult("CONCLUÍDO", correct, correct, buildDetails());
...
function violate(reason) {
  ...
  sendResult("VIOLAÇÃO - NOTA ZERO", 0, 0, details, true);
